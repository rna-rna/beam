...
export default function ProjectsPage() {
  ...
  const {
    ...
    queryKey: ['/api/galleries', searchQuery],
    queryFn: async ({ pageParam = 1 }) => {
      const token = await getToken();
      const searchParam = searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : '';
      const res = await fetch(`/api/galleries?page=${pageParam}&limit=${ITEMS_PER_PAGE}${searchParam}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  // Flatten all pages of gallery data
  ...
  const handleShare = (gallery) => {
    ReactDOM.createRoot(document.getElementById("modal-root")).render(
      <ShareModal 
        onClose={() => {
          ReactDOM.createRoot(document.getElementById("modal-root")).render(null);
        }}
        gallery={gallery}
      />
    );
  ...
  return (
    <div className="flex flex-1 bg-background">
        ...
        <header className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-2">
            <Sheet>
              ...
              <SheetContent side="left" className="w-64 overflow-hidden">
                <DashboardSidebar />
              </SheetContent>
            </Sheet>
            <div className="relative w-64">
          ...
          {status === "loading" ? (
            ...
            <>
              {galleries.length === 0 ? (
                ...
                <div className={`grid ${isListView ? '' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'} gap-4`}>
                  {galleries.map((gallery) => (
                    <ContextMenu key={gallery.id}>
                      <ContextMenuTrigger>
                        {isListView ? (
                          <div 
                            className="flex items-center p-3 border rounded-md hover:shadow cursor-pointer"
                            onClick={() => (window.location.href = `/g/${gallery.slug}`)}
                          >
                            <div className="w-16 h-16 bg-muted rounded-md flex-shrink-0 mr-4 overflow-hidden">
                              {gallery.thumbnailUrl ? (
                                <img
                                  src={getR2Image(gallery.thumbnailUrl)} 
                                  alt={gallery.title}
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center text-sm text-muted-foreground">
                                ...
                                <span>Last edited {dayjs(gallery.updatedAt).fromNow()}</span>
                              </div>
                            </div>
                          </div>
                        ) : (
                      ...
                      <ContextMenuContent>
                        ...
                        <ContextMenuItem onClick={() => setRenameGallery(gallery)}>
      ...
      {/* Modals */}
      {renameGallery && (
        <Dialog open={!!renameGallery} onOpenChange={() => setRenameGallery(null)}>
          <DialogContent>
            <RenameGalleryModal 
      ...
      {deleteGallery && (
        <Dialog open={!!deleteGallery} onOpenChange={() => setDeleteGallery(null)}>
          <DialogContent>
            <DeleteGalleryModal 
