      ...
      try {
        ...
        if (userId) {
          await addInviteNotification({
            ...
            actorAvatar: user.imageUrl || undefined,
            actorColor: user.publicMetadata?.color as string,
            galleryId: gallery.id,
            role
          });
        }
...
async function addInviteNotification(data: {
  recipientUserId: string;
  actorId: string;
  actorName: string;
  actorAvatar: string | null;
  ...
  const existingNotification = await db.query.notifications.findFirst({
    where: and(
      eq(notifications.type, 'invite-received'),
      eq(notifications.userId, data.recipientUserId),
      sql`${notifications.data}->>'galleryId' = ${data.galleryId}::text`,
      sql`${notifications.createdAt} >= NOW() - INTERVAL '5 seconds'`
    )
  });
...
async function addCommentNotification(data: {
  ...
  const existingNotification = await db.query.notifications.findFirst({
    where: and(
      eq(notifications.type, 'comment-added'),
      eq(notifications.userId, data.recipientUserId),
      sql`${notifications.data}->>'imageId' = ${data.imageId}::text`,
      sql`${notifications.createdAt} >= NOW() - INTERVAL '5 seconds'`
    )
